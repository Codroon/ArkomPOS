/**
 * The Postgres side of `CloudStore`.
 *
 ...
 *
 *  1. **A code is spent exactly once.** The claim is a conditional UPDATE, so
 *     the database decides the winner, not this process.
 ...
 *
 * Nothing acks optimistically. If any part of the write throws, the route
 * answers 500, the till keeps its cursor, and the next round sends the same
 * rows — which cost one conflict each and nothing else.
 */
...
export function pgStore(handle?: CloudDb): CloudStore {
  ...
  return {
    ...
    async recordBatch({ device, ops, appVersion, now }: RecordBatchInput): Promise<RecordBatchResult> {
      ...
      /* A healthy till cannot send the same op_id twice — its oplog has a
         unique index on it — but a VALUES list with a repeat would make the
         conflict count lie, so the dedupe is here rather than assumed. */
      ...
      /* Only now: the rows are committed, so the cursor may move. The stored
         cursor is monotonic for the dashboard's sake — a replaying till pushes
         low seqs again and "how far along is this till" should not go
         backwards — while the ACK is what this batch actually contained. */
      await db()
        .update(devices)
        .set({
      ...
      await db().update(tenants).set({ lastSeenAt: now }).where(eq(tenants.id, device.tenantId));
    ...
    async enrol(input: EnrolInput): Promise<EnrolOutcome> {
      /* Spend the code first. Whatever else goes wrong from here, it cannot be
         spent twice: the WHERE clause is the lock. */
      const claimed = await db()
        ...
        .returning({ id: enrolCodes.id, accountId: enrolCodes.accountId });
      ...
      if (existing[0] && existing[0].accountId !== code.accountId) {
        /* Hand the code back. It bought nothing, and the owner should not have
           to generate another because they pasted it into the wrong till. */
        await db().update(enrolCodes).set({ usedAt: null }).where(eq(enrolCodes.id, code.id));
      ...
      if (deviceId) {
        await db()
          ...
          .where(eq(enrolCodes.id, code.id));
      }

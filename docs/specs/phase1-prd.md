...
- [x] **2.9** (P1 subset) Completing a sale allocates the next gap-free number in the till's ticket series and prints a simplified ticket (ESC/POS; PDF fallback) showing shop data, doc number, lines, IVA breakdown, tenders, change. Reprint available. *(Done 2026-08-24: ESC/POS via the Windows RAW spooler, PDF at `userData/tickets`, reprints stamped COPIA from both the completed panel and the ticket peek. A print failure leaves the sale completed and reprintable. The thermal path gets its live test at the shop visit — the dev machine has no printer.)*
...
- [ ] **2.10 fail-fast availability** (added 2026-08-25, client-driven). *Available* = on-hand − what the current ticket already claims (`availableForSale` in core). Scanning or tapping a quantity item with available ≤ 0 gives immediate "Agotado / Out of stock" feedback (toast + the card shakes) and adds nothing; the quantity stepper and repeated scans cap at available with "Solo quedan N / Only N left". Serialized items already behave this way — the unit picker only ever offers in-stock units. *Reason:* the owner could pile a sold-out item onto a ticket and only learn at Cobrar, which wastes the customer's time at the counter. **Deliberate limits:** parked tickets do NOT hold quantity stock (only serialized units reserve), so two tickets can still claim the same last item — that race is caught by the completion-time NEGATIVE_STOCK guard, which remains the final authority. This requirement is a UX layer on top of it, never a replacement.
...
## Open questions
1. **Ahmer (blocking for go-live, not for build):** shop fiscal data for the ticket footer (name, NIF, address) + logo. *There is now somewhere to put it — Ajustes → Datos de la tienda — and the seed writes `PENDIENTE` into all three, so a test ticket names exactly what is still owed;* which receipt printer & scanner models are on the counter (ESC/POS interface: USB/network?).
